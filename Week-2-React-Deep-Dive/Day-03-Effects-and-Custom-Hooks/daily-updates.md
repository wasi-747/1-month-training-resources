# 📝 Daily Progress Updates — Week 2, Day 3 (Wednesday)

## 📋 TODO List — Wednesday (Week 2, Day 3)
1. 🎮 Complete the full interactive Tic-Tac-Toe game logic with historical time-travel moves.
2. 🔄 Study React Side Effects & the `useEffect` Hook (Synchronizing with external systems).
3. 📦 Understand Dependency Arrays: Learn empty arrays `[]` vs specific state dependencies.
4. 🧹 Learn Component Cleanup: Prevent memory leaks by clearing timers/event listeners.
5. ⚡ Build a custom data-fetching Hook (`useFetch`) for cleaner code reusability.
6. 💻 Commit practice components, study notes, and daily logs to GitHub.
